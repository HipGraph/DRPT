...
//        vector<vector<VALUE_TYPE>> readImages(vector<string> imagePaths);
        ...
        vector<vector<VALUE_TYPE>> mpi_file_read(string path, int rank, int world_size, int overlap,int total_data_set_size, int data_node_byte, int offset, int dimension);
