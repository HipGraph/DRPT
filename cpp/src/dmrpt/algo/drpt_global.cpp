...
//    cout << " Ending API gatherV mean calc rank " << this->rank << endl;
    ...
    this->send_receive_data_points_if_zero(left_childs_global, right_childs_global, total_counts, process_counts, disps,
                                           depth, tree);
...
void dmrpt::DRPTGlobal::send_receive_data_points_if_zero(vector <DataPoint> left_data_points,
                                                         ...
                                                         int *total_counts, int *process_counts, int *disps, int depth,
                                                         int tree) {
    ...
    for (int i = 0; i < this->world_size; i++) {
        ...
        if (this->is_transfer_needed(total_counts,0)) {

            this->send_receive_data_points_if_zero(left_data_points, total_counts, i, 0, depth, tree);
//            cout<<"Calling MPIGatherV left "<<this->rank<<endl;
            MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_INT, total_counts, process_counts, disps, MPI_INT, MPI_COMM_WORLD);
//            cout<<"Exit MPIGatherV left "<<this->rank<<endl;
        } else if (this->is_transfer_needed(total_counts,1)) {
//            cout << "calling right rank" << this->rank << endl;
            this->send_receive_data_points_if_zero(right_data_points, total_counts, i, 1, depth, tree);
//            cout<<"Calling MPIGatherV right "<<this->rank<<endl;
            MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_INT, total_counts, process_counts, disps, MPI_INT, MPI_COMM_WORLD);
//            cout<<"Exit MPIGatherV right "<<this->rank<<endl;
        }
    }
}
