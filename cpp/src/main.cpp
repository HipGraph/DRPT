...
int main(int argc, char *argv[]) {
    ...
    MDRPT mdrpt = MDRPT(1,imagedatas,tree_levels,60000,10,15,dmrpt::StorageFormat::RAW,rank,size);
...
//     vector<vector<dmrpt::DRPT::DataPoint>> results =  mdrpt.batch_query(1000,5000.0,3,10);
     ...
     vector<DataPoint> data_points= mdrpt.get_knn(10);
...
//                        for (int l = 0; l < results[k].size(); l++) {
                        if (data_points[k].src_index != data_points[k].index) {

                                fout << data_points[k].src_index +1 << ' ' <<data_points[k].index +1<< endl;
                        }
//                        }
                    }
                }
