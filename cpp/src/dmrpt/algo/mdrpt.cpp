...
void
dmrpt::MDRPT::grow_trees(vector <vector<VALUE_TYPE>> &original_data, float density, bool use_locality_optimization) {
...
std::map<int, vector < dmrpt::DataPoint>> dmrpt::MDRPT::communicate_nns(map<int, vector < dmrpt::DataPoint>>
                              &local_nns,
                              int nn) {
...
#pragma omp parallel for
    for (int i = 0;i < local_nns.size();i++) {
        sending_indices[i] = local_nns[i];
        sending_max_dist_thresholds[i] = local_nns[i][nn - 1].distance;
//        count++;
    }
